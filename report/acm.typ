...
// it as an article in the style of the ACM (adapted from IEEE).
#let acm(
  // The paper's title.
  title: [Paper Title],
  ...
  // Set the body font.
  set text(font: "Linux Libertine", size: 9pt, spacing: .35em)
  ...
  show figure.where(kind: table): set figure(numbering: "1", supplement: "Table")
  ...
  set figure(placement: none) // TODO change if needed
  ...
  show heading: it => {
    ...
    if it.level == 1 or it.level == 2{
      show: block.with(above: 13.5pt, below: 9pt, sticky: true)
  ...
  set par(justify: true, first-line-indent: (amount: 1em, all: false), spacing: 0.5em, leading: 0.5em)
